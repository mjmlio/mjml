...
| `mjml [input] --config.filePath`                        | Sets the base directory for resolving `mj-include` paths and scoping the include sandbox. Relative paths in `--config.includePath` are resolved from this directory. Useful with `--stdin` when there is no input file on disk.    | _directory of the input file_                                                      |
| `mjml [input] --config.includePath`                     | Adds allowlisted include root(s), as a string path or JSON array of paths. Relative paths are resolved from the CLI working directory.                                                                                             |                                                                                    |
...
| `mjml [input] --config.stack`                           | Prints the full error stack trace on compilation errors (`true` or `false`)                                                                                                                                                        | `false`                                                                            |
...
- **Canonical boundary checks:** we resolve and follow symlinks and require the target to stay inside `filePath` or any `includePath` roots.
