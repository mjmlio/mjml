...
class Chart extends Component {
  // parentWidth = this.props.mjAttribute('parentWidth')
  ...
  render () {
    ...
    // ensure forbidden attributes were not specified
    forEach(this.props.mjml.get('attributes').toObject(), (value, attr) => {
      assert(!forbiddenAttributes.includes(attr), `"${attr}" is not an authorized attribute for <mjml-chart>`);
    })

    // build url
    const url = buildURL(mjAttribute)
    return (<Image
      width={width}
      height={height}
      src={url}
