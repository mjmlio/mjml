import { find, filter, get, identity, map, omit, reduce, isObject, each, isEmpty } from 'lodash'
...
import MJMLValidator, { dependencies as globalDependencies, assignDependencies } from 'mjml-validator'

import { BodyComponent, HeadComponent, initComponent } from './createComponent'
import globalComponents, { registerComponent, assignComponents } from './components'
...
type MjmlOptions = {
  ...
  minify?: boolean
  minifyOptions?: any // infer from prettier
  ignoreIncludes?: boolean
  juiceOptions?: any
  juicePreserveTags?: any // infer from juice
  skeleton: string | typeof defaultSkeleton
  validationLevel?: 'soft' | 'strict' | 'skip'
  filePath?: string
  actualPath?: string
  preprocessors: Array<(input: string) => string>
  presets: Array<{
    components: Array<HeadComponent | BodyComponent>
...
type MjmlStructure = {
  ...
  children?: MjmlStructure[]
  content?: string
}

export default async function mjml2html(inputMjml: string | MjmlStructure, options: MjmlOptions) {
  let content = ''
  let errors: Array<{ formattedMessage: string }> = []

  if (isNode && typeof options.skeleton === 'string') {
    /* eslint-disable global-require */
    /* eslint-disable import/no-dynamic-require */
    options.skeleton = require(
      options.skeleton.charAt(0) === '.' ? path.resolve(process.cwd(), options.skeleton) : options.skeleton
    )
  ...
  const mjml: MjmlStructure =
    typeof inputMjml === 'string'
      ? MJMLParser(inputMjml, {
          keepComments,
          components,
          filePath,
          actualPath,
  ...
  if (mjOutsideRaws.length) {
    const toAddBeforeDoctype = mjOutsideRaws.filter(
      elt => elt.attributes.position && elt.attributes.position === 'file-start'
    )
