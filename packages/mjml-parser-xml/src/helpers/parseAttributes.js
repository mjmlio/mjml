...
export default function parseAttributes(input) {
  ...
  return input.replace(regexTag, match =>
    match.replace(regexAttributes, (m, beforeAttr, attrVal, afterAttr) => {
...
export function decodeAttributes(input) {
  return input.replace(regexTag, match =>
    match.replace(regexAttributes, (match, beforeAttr, attrVal, afterAttr) => {
