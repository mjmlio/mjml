...
| aria-hidden             | string                  | when set to `true` adds an `aria-hidden` attribute to the icon image or `aria-hidden` and `tabindex` to the parent `<a>` (if `href` is set) | `true` (if text is included)           |
